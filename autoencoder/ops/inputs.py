...
def parse_sequence_example(serialized, image_feature, sentences_feature, sentence_length):
  """Parses a tensorflow.SequenceExample into an image and text.
  ...
  return encoded_image, sentences
...
def batch_with_dynamic_pad(images_and_texts,
                           ...
                           queue_capacity,
                           add_summaries=True):	
  """Batches input images and texts.
  ...
  Args:
    images_and_texts: A list of pairs [image, texts], where image is a
  ...
  Returns:
    ...
    target_seqs: An int32 Tensor of shape [batch_size, padded_length, sentence_length].
    mask: An int32 0/1 Tensor of shape [batch_size, padded_length].
  """
  enqueue_list = []
  for image, text in images_and_texts:
  ...
  images, input_seqs, mask = tf.train.batch_join(
      enqueue_list,
  ...
  if add_summaries:
    ...
    tf.scalar_summary("text_length/batch_min", tf.reduce_min(lengths))
    tf.scalar_summary("text_length/batch_max", tf.reduce_max(lengths))
    tf.scalar_summary("text_length/batch_mean", tf.reduce_mean(lengths))
